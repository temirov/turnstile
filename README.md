# Turnstile Browser Gateway — Turnstile + JWT + DPoP + Tiny JS SDK

Turnstile is a compact Go **gateway** that sits in front of any HTTP API so a
...
* **Zero crypto in your app.** Turnstile serves a **tiny JS SDK** that handles keygen, token minting, and DPoP.

---

## What’s in the box

* **Turnstile service (Go)** with two endpoints: `/tvm/issue` and `/api`.
...
Turnstile ── verifies Origin + Turnstile ──► mint HS256 JWT (5 min, cnf.jkt)

Browser   ──(Bearer JWT + DPoP)────────────►  POST /api
Turnstile ── Origin + rate + JWT + DPoP ───► reverse-proxy to upstream
```
...
> **Important:** `UPSTREAM_BASE_URL` is the **base origin only** (no path). Turnstile preserves `/api`.
...
Keep CORS in **Turnstile**. Ensure the proxy passes the client scheme (`X-Forwarded-Proto`) so DPoP `htu` matches.
...
server {
  ...
  server_name turnstile.mprlab.com;

  # ssl_certificate ...;  ssl_certificate_key ...;

  proxy_set_header X-Forwarded-Proto $scheme;

  location /tvm/issue { proxy_pass http://turnstile:8080/tvm/issue; }
  location /api       { proxy_pass http://turnstile:8080; }   # no rewrite needed
...
```caddy
turnstile.mprlab.com {
	encode zstd gzip

	handle_path /tvm/issue* {
		reverse_proxy http://turnstile:8080 {
...
*If the proxy and Turnstile share a Docker network, point to `http://turnstile:8080`.*

---

## Front-end usage (with the built-in SDK)

You don’t write crypto. Import the module Turnstile serves and call **one function**.
...
<script type="module">
  ...
  // Call your protected upstream via Turnstile:
...
* **Capability token**: HS256 JWT, audience-scoped to Turnstile, TTL ≈ 5 minutes.
* **Proof-of-possession**: Token carries `cnf.jkt` (JWK thumbprint). Each request must present a **DPoP** JWS signed by that key; Turnstile verifies method (`htm`) and URL (`htu`).
* **Bot friction**: Optional **Turnstile** at token issuance (not per request).
* **Replay defense**: In-memory `jti` cache until expiry.
* **Origin enforcement**: Exact allowlist; CORS headers added by Turnstile.
...
* To protect multiple upstream routes, just call `postJson(payload, { path: "/api/whatever" })`. Turnstile applies the same checks before proxying.
...
* **`htu_mismatch`** → Ensure the reverse proxy sets `X-Forwarded-Proto` correctly; the browser URL must match what Turnstile computes.
